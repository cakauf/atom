...
from atom.apikey import api_key, url

model_name = None

if isinstance(api_key, list):
    clients = [openai.Client(base_url=url, api_key=key) for key in api_key]
else:
    clients = [openai.Client(base_url=url, api_key=api_key)]
...
def gen(msg, model=None, temperature=None, response_format="json_object"):
    ...
    for retry in range(MAX_RETRIES):
        try:
            ...
            errors.append(f"API error: {e!s}")
        except Exception as e:
            errors.append(f"Error: {type(e).__name__}, {e!s}")

        import time

        time.sleep(DEFAULT_RETRY_AFTER * (2**retry))
